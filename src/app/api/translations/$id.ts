...
const createSavedResponse = (payload: {
    ...
    path: string;
    stale?: boolean;
}) => Response.json({ ...payload, saved: true }, payload.duplicate ? { status: 409 } : undefined);
...
const writeMetaFile = async (
    ...
    await writeTextFile(metaPath, JSON.stringify(writeMeta, null, 2));
    return writeMeta;
};
...
const handleIdempotentWrite = async (params: {
    ...
    await writeTextFile(params.outputPath, params.rawConversation);
    ...
    await writeTextFile(idempotencyPath, JSON.stringify(idempotencyRecord, null, 2));
...
export const POST = async (request: Request, context: RouteContext) => {
    try {
        ...
        await writeTextFile(outputPath, rawConversation);
...
export const Route = createFileRoute('/api/translations/$id')({
    server: { handlers: { POST: ({ params, request }) => POST(request, { params: Promise.resolve(params) }) } },
});
