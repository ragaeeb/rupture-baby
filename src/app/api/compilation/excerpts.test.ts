...
mock.module('@/lib/prompt-state', () => ({ getSelectedPrompt: async () => state.prompt }));
...
const { GET } = await import('./excerpts');

describe('GET /api/compilation/excerpts', () => {
    it('should return paginated untranslated excerpts data', async () => {
        const response = await GET(
            new Request('http://localhost/api/compilation/excerpts?page=1&pageSize=1&maxIds=10'),
        );
        const json = (await response.json()) as {
            data: MockExcerpt[];
            pagination: { totalItems: number };
            picker: { displayedTotal: number };
        };
    ...
    it('should return 400 when compilation path is missing', async () => {
        ...
        const { GET: GetWithError } = await import(`./excerpts?case=missing-path-${Date.now()}`);
