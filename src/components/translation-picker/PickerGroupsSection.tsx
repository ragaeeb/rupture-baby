...
const PickerGroupsSection = ({
    ...
    const displayedIdIndexLookup = useMemo(
        () => new Map(picker.displayedIds.map((id, index) => [id, index])),
        [picker.displayedIds],
    );
                    ...
                    <tbody>
                        {picker.tokenGroups.map((group) => (
                            <tr key={group.label} className="border-b last:border-b-0">
                                <td className="w-28 border-r bg-neutral-50 px-3 py-2 align-top text-sm">
                                    {group.label}
                                </td>
                                <td className="p-3">
                                    <div className="flex flex-wrap gap-2">
                                        {group.ids.map((id) => (
                                            <button
                                                ...
                                                onClick={() => {
                                                    const index = displayedIdIndexLookup.get(id);
                                                    if (index !== undefined) {
