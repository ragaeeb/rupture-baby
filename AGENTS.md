...
- a compilation export page for chunked JSON attachment downloads
...
- `src/components/compilation-export-page.tsx`
...
- `_browse` is pathless. Public URLs are `/analytics`, `/compilation`, `/exports`, `/shift`, `/valid`, `/invalid`, and `/translations/...`, not `/_browse/...`.
...
- `COMPILATION_FOLDER` may contain multiple compilation JSON files; the active one is selected in the UI and persisted in `.<folder>/.active-compilation.json`.
- Only visible top-level `.json` files in `COMPILATION_FOLDER` count as selectable compilations. Hidden sidecars like `.*.settings.json` and `.*.stats-cache.json` must be ignored.
- The active compilation file can be very large, often hundreds of MB.
...
- A disposable compilation snapshot cache may be persisted beside the compilation file as `.<compilation-name>.compilation-cache.json`.
